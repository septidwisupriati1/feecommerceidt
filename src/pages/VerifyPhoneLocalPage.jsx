...
export default function VerifyPhoneLocalPage() {
  ...
  useEffect(() => {
    ...
    setPhone(user?.phone || '+62xxxxxxxxxx');
  }, []);

  const sendCode = () => {
    if (secondsRemaining > 0) return;
    setStep('sent');
    setToast({ show: true, message: `Kode verifikasi telah dikirim ke ${phone} (simulasi)`, variant: 'success' });
  ...
  const handleVerify = (e) => {
    ...
    setStep('verified');
    setToast({ show: true, message: 'Nomor telepon berhasil diverifikasi (simulasi)', variant: 'success' });
  };
        ...
        <div className="max-w-md mx-auto">
          <Card>
            <CardContent className="p-6">
              <h2 className="text-2xl font-bold mb-2">Verifikasi Nomor Telepon</h2>
              <p className="text-gray-600 mb-6">Halaman ini hanya tampilan front-end; SMS tidak dikirimkan sebenarnya.</p>

              {step === 'idle' && (
                <div className="space-y-4">
                  <p className="text-sm text-gray-700">Kami akan mengirimkan kode verifikasi ke nomor:</p>
                  <div className="p-3 bg-gray-50 rounded-md font-medium">{phone}</div>
              ...
              {step === 'sent' && (
                <form onSubmit={handleVerify} className="space-y-4">
                  <p className="text-sm text-gray-700">Masukkan kode yang Anda terima via SMS:</p>
              ...
              {step === 'verified' && (
                <div className="text-center space-y-4">
                  <div className="text-green-600 font-semibold">✔ Nomor telepon terverifikasi (simulasi)</div>
